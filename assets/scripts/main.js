...
async function getRecipes() {
  ...
  {
    let recipies = localStorage.getItem('recipes');
  ...
  return new Promise(fetchRecipies); 
  async function fetchRecipies(resolve, reject) {
  /**************************/
  ...
  //            will be in the catch portion.
    ...
    //            resolve() method.
        if(i == RECIPE_URLS.length - 1){
          saveRecipesToStorage(fetchedRecipies);
          resolve(fetchedRecipies);
        }
      }
      catch(err){
